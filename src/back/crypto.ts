...
function encryptWithPbkdf2(data: any, password: string) {
...
function decryptWithPbkdf2(buffer: Buffer, password: string) {
...
function encryptWithKey(data: any, key: Buffer) {
    const stringified = JSON.stringify(data)
    const iv = randomBytes(ivLen)
    const cipher = createCipheriv(algorithm, key, iv)
    const encrypted = Buffer.concat([cipher.update(stringified), cipher.final()])
...
function decryptWithKey(buffer: Buffer, key: Buffer) {
...
export function createBip39Seed() {
